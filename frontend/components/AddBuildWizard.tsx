...
export default function AddBuildWizard({ isOpen, onClose, onSuccess }: Props) {
  ...
  const handleFileSelected = async (file: File) => {
    ...
    setUploadError(null);
    try {
      const result = await uploadIngestFile(file, selected.id);
        ...
        >
              ...
              {connectors.map((c) => {
                ...
                return (
                  <button
                    ...
                    className="flex items-start gap-3 rounded-lg border border-slate-200 p-3 text-left hover:border-cyan-500/40 hover:bg-cyan-50/50 transition-all dark:border-white/[0.08] dark:hover:border-cyan-500/30 dark:hover:bg-cyan-500/[0.04]"
                  >
                    <div className="rounded-lg bg-slate-100 p-2 dark:bg-white/[0.06]">
            ...
            <div>
                      ...
                      >
                        {isUploading ? (
                          <>
                            ...
                            <p className="text-xs text-slate-500 dark:text-white/40">Uploading...</p>
                          </>
                        ) : uploadedFile ? (
                          <>
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="rounded-lg px-4 py-2 text-xs font-semibold text-white bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
          ...
          {step === "running" && (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="text-sm font-semibold text-slate-700 dark:text-white/80">Ingesting build{buildId ? ` ${buildId}` : ""}...</p>
              <p className="text-xs text-slate-400 dark:text-white/30">{pollElapsed}s elapsed</p>
              <p className="text-[11px] text-slate-400 dark:text-white/30 max-w-xs">
              ...
              <button
                onClick={handleClose}
                className="mt-1 rounded-lg px-3 py-1.5 text-xs font-semibold border border-slate-300 text-slate-700 hover:bg-slate-100 dark:border-white/[0.12] dark:text-white/70 dark:hover:bg-white/[0.07]"
              >
                Close
              </button>
            </div>
