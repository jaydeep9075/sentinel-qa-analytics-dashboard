...
export default function IngestionSelector() {
  ...
  // The confirm dialog is portalled to <body> for the same reason
  // AddBuildWizard is (see the comment there): this component lives in the
  // dashboard header's `controls` slot, which is `overflow-x-auto` - and CSS
  // computes `overflow-y: visible` to `auto` as soon as the other axis isn't
  // visible, so the slot clips vertically too. On top of that the header
  // carries `backdrop-blur-xl`, and a backdrop-filter makes `position: fixed`
  // resolve against that ancestor instead of the viewport. Rendered in place,
  // the dialog was therefore laid out inside a 44px-tall clipping box and was
  // simply never visible - which is what made "delete build" look dead.
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);

  // Escape closes, but not mid-request: the DELETE is already in flight and
  // dropping the dialog would hide the error if it comes back a refusal.
  useEffect(() => {
  ...
  const confirmTarget = ingestions.find((i) => i.id === deleteConfirm);
          ...
          >
            ...
            <p className="text-sm text-slate-600 dark:text-white/60 mb-4">
              ...
              <span className="font-semibold text-slate-900 dark:text-white">
                {confirmTarget?.display_name || confirmTarget?.build_label || "this build"}
              </span>{" "}
              and all its related data? This action cannot be undone.
            </p>
  ...
  return (
    <div className="relative flex shrink-0 items-center gap-1.5">
      ...
      {/* Delete button - hidden for roles without data.delete; server-side
          ...
          Opens the confirm dialog directly. There used to be a small popover
          in between asking the same "this cannot be undone" question the
          dialog asks, which was both redundant and the piece the header's
          overflow clipped - one portalled confirmation is the whole gate. */}
      {canDelete && (
        <button
          onClick={() => selectedIngestion && setDeleteConfirm(selectedIngestion)}
          disabled={!selectedIngestion}
          className="p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-100/20 disabled:opacity-40 disabled:cursor-not-allowed dark:text-white/40 dark:hover:text-red-400 dark:hover:bg-red-500/10 transition-all"
