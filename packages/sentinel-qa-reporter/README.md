...
Stream a **Playwright** or **Cypress** run to a [Sentinel QA Analytics](../../README.md)
...
One package, both frameworks, two environment variables.

```bash
npm i -D sentinel-qa-reporter
```

If your team hosts Sentinel itself, install the copy that host was built with
...
|---|---|---|
| `SENTINEL_URL` | yes | Sentinel API origin. **Nothing reports without it** — which is what makes the config safe to commit. |
