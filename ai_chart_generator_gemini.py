...
class AIChartGenerator:
    ...
    def generate_chart(self, prompt: str) -> Dict[str, Any]:
        if not self.use_gemini: return {"success": False, "error": "AI not configured"}
        try:
            # Giving Gemini the data structure
    ...
    def generate_chat_response(self, message: str) -> str:
        if not self.use_gemini: return "AI disabled."
        
        # Prepare context for the random question
        summary = {
            "total": len(self.df),
...
GEMINI_API_KEY = "AIzaSyD1wDd6FZlinwfeaiUFmGLzHjqsnrw-bGM"
MASTER_FILE = "qa_analytics_master.json"
generator = AIChartGenerator(MASTER_FILE, GEMINI_API_KEY)

class ChartReq(BaseModel): prompt: str
